...
init_battleChatter()
{
	...
	if (getcvar ("bcs_enable") == "")
		setcvar ("bcs_enable", "on");

	if (getcvar ("bcs_enable") == "off")
	{
	...
	if (getcvar ("bcs_filterThreat") == "")
		setcvar ("bcs_filterThreat", "off");
	if (getcvar ("bcs_filterInform") == "")
		setcvar ("bcs_filterInform", "off");
	if (getcvar ("bcs_filterOrder") == "")
		setcvar ("bcs_filterOrder", "off");
	if (getcvar ("bcs_filterReaction") == "")
		setcvar ("bcs_filterReaction", "off");
	if (getcvar ("bcs_filterResponse") == "")
		setcvar ("bcs_filterResponse", "off");

	if (getcvar ("bcs_threatLimitTargettedBySelf") == "")
		setcvar ("bcs_threatLimitTargettedBySelf", "off");
	if (getcvar ("bcs_threatLimitTargetingPlayer") == "")
		setcvar ("bcs_threatLimitTargetingPlayer", "off");
	if (getcvar ("bcs_threatLimitInPlayerFOV") == "")
		setcvar ("bcs_threatLimitInPlayerFOV", "on");
	if (getcvar ("bcs_threatLimitInLocation") == "")
		setcvar ("bcs_threatLimitInLocation", "on");
	if (getcvar ("bcs_threatLimitSpeakerDist") == "")
		setcvar ("bcs_threatLimitSpeakerDist", "512");
	if (getcvar ("bcs_threatLimitThreatDist") == "")
		setcvar ("bcs_threatLimitThreatDist", "2048");

	if (getcvar ("bcs_threatPlayerRelative") == "")
		setcvar ("bcs_threatPlayerRelative", "off");

	if (getcvar("debug_bcprint") == "")
		setcvar("debug_bcprint", "off");
	if (getcvar("debug_bcshowqueue") == "")
		setcvar("debug_bcshowqueue", "off");
/#
	if (getcvar("debug_bcthreat") == "")
		setcvar("debug_bcthreat", "off");
	if (getcvar("debug_bcresponse") == "")
		setcvar("debug_bcresponse", "off");
	if (getcvar("debug_bcorder") == "")
		setcvar("debug_bcorder", "off");
	if (getcvar("debug_bcinform") == "")
		setcvar("debug_bcinform", "off");
	if (getcvar("debug_bcdrawobjects") == "")
		setcvar("debug_bcdrawobjects", "off");
	if (getcvar("debug_bcinteraction") == "")
		setcvar("debug_bcinteraction", "off");
#/
...
// will make it override pre-existing events of the same type.
	...
	*/
	...
	anim.eventPriority["response"]["killconfirm"] 	= 0.8;
	anim.eventPriority["response"]["acknowledge"] 	= 0.9;
	...
	anim.eventPriority["inform"]["killconfirm"] 	= 0.6;
	anim.eventPriority["inform"]["attacking"] 		= 0.9;
	...
	anim.eventDuration["response"]["killconfirm"] 	= 3000;
	anim.eventDuration["response"]["acknowledge"]   = 2000;
	...
	anim.eventDuration["inform"]["killconfirm"] 	= 1000;
	anim.eventDuration["inform"]["attacking"] 		= 1000;
...
/#
	if (getcvar("debug_bcdrawobjects") == "on")
...
#/
	...
	anim.lastTeamSpeakTime["allies"] = 0;
	anim.lastTeamSpeakTime["axis"] = 0;
...
bcsDebugWaiter()
{
	lastState = getcvar ("bcs_enable");

	while (1)
	{
		state = getcvar ("bcs_enable");
...
playBattleChatter()
{
	...
	validEvents = self prioritizeEvents ();
	
	if (validEvents.size == 0)
		return;
	
	switch (validEvents[0])
	{
	case "custom":
...
playThreatEvent()
{
	...
	/#
	if (getcvar("debug_bcinteraction") == "on")
...
//	else if (threat isSniper())
	if (threat isSniper())
	{
		chatPhrase threatInfantrySniper (threat);
	}
		...
		else
		{
			...
			if (chatPhrase addLocationAlias (1.0, threat))
			{
				...
				if (isdefined (orderTo))
				{
					...
					if (self isOfficer())
						orderTo addResponseEvent("acknowledge", "yes", self, 0.9);
					else
						orderTo addResponseEvent("acknowledge", "norankyes", self, 0.9);
				}
				else
				{
...
playResponseEvent()
{
	...
	if (self.chatQueue["response"].modifier == "follow" && self.anim_state != "move" )
		return;
...
responseAcknowledge (respondTo, modifier)
{
	...
	chatPhrase = self createChatPhrase();
	chatPhrase addResponseAlias (1.0, "acknowledge", modifier);
...
responseKillConfirm (respondTo, modifier)
{
	...
	chatPhrase = self createChatPhrase();
	chatPhrase addResponseAlias (1.0, "killconfirm", "infantry");
	chatPhrase.lookTarget = respondTo;
...
orderMove (modifier, orderTo)
{
	...
	if (modifier == "follow")
	{
		...
		for (i = 0; i < soldiers.size; i++)
		{
			...
			if (distance (self.origin, soldiers[i].origin) > 500 && soldiers[i].anim_state == "move" && soldiers[i] canSay("response"))
				orderTo = soldiers[i];
		}
		
		/#
		if (getcvar("debug_bcinteraction") == "on" && isDefined( orderTo ) )
		...
		if (isdefined (orderTo) && isdefined (orderTo.bcName) && self canSayName(orderTo.bcName))
		{
			chatPhrase addNameAlias (orderTo.bcName);
			chatPhrase.lookTarget = orderTo;
			orderTo addResponseEvent("acknowledge", "follow", self, 1.0);
		}
...
informReloading (informTo, modifier)
{
	...
	if (!isdefined (modifier) || (modifier == "coverme" && !isalive (informTo)))
		modifier = "generic";

	if (modifier == "coverme" && isalive (informTo))
	{
		if (informTo == level.player)
		{
			chatPhrase addRankAlias ("private");
			return;
		}
			
		if (isdefined (informTo.bcName) && self canSayName(informTo.bcName))
			chatPhrase addNameAlias (informTo.bcName);
		else if (isdefined (informTo.bcRank))
			chatPhrase addRankAlias (informTo.bcRank);
			
		informTo addResponseEvent ("acknowledge", "covering", self, 0.9);
...
informAttacking (modifier)
{
	...
	chatPhrase = self createChatPhrase();	
	chatPhrase addInformAlias (1.0, "attacking", modifier);
...
informKillConfirm (modifier)
{
	...
	chatPhrase = self createChatPhrase();	
	chatPhrase addInformAlias (1.0, "killconfirm", "infantry");
...
playPhrase (chatPhrase, noSound)
{
	...
	self endon ("death");
//	if (isdefined (chatPhrase.lookTarget))
//		self animscripts\shared::LookAtEntity(chatPhrase.lookTarget, 2, "alert", undefined, "interrupt");
	...
	for (i = 0; i < chatPhrase.soundAliases.size; i++)
	{
		...
		{
			if (getcvar ("bcs_threatPlayerRelative") == "on")
			{
...
lockAction (speaker, eventAction, customEvent )
{
	...
	message = speaker waittill_any ("death", "done speaking", "cancel speaking");
	...
	if (isalive (speaker))
	{
		...
		speaker.chatQueue[eventAction].expireTime = 0;
		speaker.chatQueue[eventAction].priority = 0.0;
		speaker.nextSayTimes[eventAction] = gettime() + anim.eventActionMinWait[eventAction]["self"];
	}
...
updateContact (squadName, member)
{
	if (gettime() - self.squadList[squadName].lastContact > 10000)
	{
		...
		{
			if (self.members[i] != member && isalive (self.members[i].enemy) && self.members[i].enemy.squad.squadName == squadName)
				isInContact = true;
		}
...
canSay (eventAction, eventType, priority)
{
	self endon ("death");
	self endon ("removed from battleChatter");
...
prioritizeEvents()
{
	...
	if (self isValidEvent ("custom"))
		validEvents[validEvents.size] = "custom";
	if (self isValidEvent ("response"))
		validEvents[validEvents.size] = "response";
	if (self isValidEvent ("order"))
		validEvents[validEvents.size] = "order";
	if (self isValidEvent ("threat"))
		validEvents[validEvents.size] = "threat";
	if (self isValidEvent ("inform"))
		validEvents[validEvents.size] = "inform";
	if (self isValidEvent ("reaction"))
		validEvents[validEvents.size] = "reaction";
	
	for (i = validEvents.size - 1; i >= 0; i--)
	{
		for (j = 1; j <= i; j++)
		{
			if (self.chatQueue[validEvents[j-1]].priority < self.chatQueue[validEvents[j]].priority)
			{
				strTemp = validEvents[j - 1];
				validEvents[j - 1] = validEvents[j];
				validEvents[j] = strTemp;
			}
		}
	}
...
{
	if (getcvar ("bcs_filter" + strAction) == "on" || getcvar ("bcs_filter" + strAction) == "1")
		return (true);
...
debugPrintEvents()
{
	...
	if (getcvar ("debug_bcshowqueue") != self.team && getcvar ("debug_bcshowqueue") != "all")
		return;
...
debugQueueEvents ()
{
	if (getcvar("debug_bcresponse") == "on")
		self thread printQueueEvent ("response");
	if (getcvar("debug_bcthreat") == "on")
		self thread printQueueEvent ("threat");
	if (getcvar("debug_bcinform") == "on")
		self thread printQueueEvent ("inform");
	if (getcvar("debug_bcorder") == "on")
...
{
	if (getcvar ("debug_bcprint") == self.team || getcvar ("debug_bcprint") == "all")
		return (true);

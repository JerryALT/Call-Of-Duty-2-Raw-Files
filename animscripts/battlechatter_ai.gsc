...
addToSystem (squadName)
{
	...
	{
		if (issubstr(tolower(self.script_friendname), "macgregor"))
			self.npcID = "mcg";
		else if (issubstr(tolower(self.script_friendname), "braeburn"))
			self.npcID = "bra";
		else if (issubstr(tolower(self.script_friendname), "mccloskey"))
			self.npcID = "mcc";
		else if (issubstr(tolower(self.script_friendname), "price"))
			self.npcID = "pri";
		else
		{
			self setNPCID();
...
aiThreadThreader()
{
	...
	if (issubstr(self.classname, "mg42portable"))
	...
	while (1)
	{
		...
		for (i = 0; i < self.members.size; i++)
		{
			...
			for (j = 0; j < validEnemies.size; j++)
			{
				...
				if (getcvar ("bcs_threatLimitTargettedBySelf") == "on")
				{
				...
				if (getcvar ("bcs_threatLimitSpeakerDist") != "off")
				{
					maxDist = int(getcvar ("bcs_threatLimitSpeakerDist"));
...
filterThreats(potentialThreats)
{
	...
	for (i = 0; i < potentialThreats.size; i++)
	{
		...
		if (getcvar ("bcs_threatLimitTargetingPlayer") == "on")
		{
			if (!isdefined (potentialThreats[i].enemy) || potentialThreats[i].enemy != level.player)
				continue;
		}

		if (getcvar ("bcs_threatLimitInPlayerFOV") == "on")
		{
			if (!level.player pointInFov(potentialThreats[i].origin))
				continue;
		}

		if (getcvar ("bcs_threatLimitThreatDist") != "off")
		{
			maxDist = int(getcvar ("bcs_threatLimitThreatDist"));
			if (distance (level.player.origin, potentialThreats[i].origin) > maxDist)
				continue;
			
		}

		if (getcvar ("bcs_threatLimitInLocation") == "on")
		{
			if (!isdefined (potentialThreats[i] getLocation()) && !isdefined (potentialThreats[i] getLandMark()))
				continue;
		}
...
/*	
	while (1)
	{
		...
		if (isdefined (self.enemy))
			self addThreatEvent ("infantry", self.enemy);
	}
*/
}
...
aiDeathEnemy ()
{
		...
		gettime() < self.calledOut[attacker.squad.squadName].expireTime)
	{
		...
		else
		{
			...
			attacker addResponseEvent ("killconfirm", "infantry", self.calledOut[attacker.squad.squadName].spotter);
		}
	}
...
aiKillEventThread()
{
	...
	wait ( 1.5 );
	self addInformEvent ("killconfirm", "infantry");
}
...
aiGrenadeDangerWaiter()
{
	...
	while (1)
	{
		...
		{
			if (distance (grenades[i].origin, level.player.origin) < 512) // grenade radius is 220
			{
...
aiDisplaceWaiter()
{
	...
	while ( true )
	{
		...
		if (gettime() < self.anim_painTime + 4000)
			continue;
		
		self addResponseEvent( "acknowledge", "yes", level.player, 1.0 );
	}
}
...
portableMG42Waiter()
{
	...
	while (1)
	{
		self waittill ("mg42_portable_setup");
...
aiFollowOrderWaiter()
{
	...
	while ( true )
	{
		...
		if (distance(self.origin, speaker.origin) < 600)
			self addResponseEvent("acknowledge", "follow", speaker);
	}
}
...
evaluateReloadEvent()
{
	...
	if (isdefined(soldier) && isdefined (soldier.anim_personImMeleeing))
		soldier = undefined;
...
evaluateAttackEvent(type)
{
	...
	{
		case "grenade":
		self addInformEvent ("attacking", "grenade");
		break;
	}
}
	...
	if (squad.squadStates["suppressed"].isActive)
	{
		...
		else if (squad.squadStates["cover"].isActive)
		{
			if (randomfloat (1) > 0.5)
				...
				self addOrderEvent ("action", "smoke");
		}
		else
		{
			if (randomfloat (1) > 0.5)

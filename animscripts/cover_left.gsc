...
main()
{
	...
	if (cornerType == "newCorner")
		animscripts\cover_left_axis::main();
	else
	if (cornerType == "noWall")
		animscripts\cover_left_axis::noWall();
...
Anims_StandingPistol()
{
	...
	animarray["anim_alert"]					= %pistol_leftstand_hide_idle;	// TODO Can also use %pistol_rightstand_hide_twitch here
...
SubState_StandingCorner(changeReason, cornerAngle, nodeOrigin)
{
	...
	if ( (self.anim_pose == "crouch") || (self.anim_pose == "prone") )	// TODO: Prone should have its own animation.
	{
		...
		{
			animscripts\SetPoseMovement::PlayTransitionAnimation(%cornercrouch2stand_right, "stand", "stop", 0);
		}
		else
		{
			animscripts\SetPoseMovement::PlayTransitionAnimation(%cornerb_crouch2stand_right, "stand", "stop", 0);
		}
	}
...
Anims_CrouchingRifleA()
{
	...
	animarray["anim_grenade"]				= %corner_crouch_grenade_right;
...
SubState_CrouchingCorner(changeReason, cornerAngle, nodeOrigin)
{
	...
	else if ( (self.anim_pose == "stand") || (self.anim_pose == "prone") )	// TODO: Prone should have its own animation.
	{
		...
		if (self.anim_idleset == "a")
			...
			animscripts\SetPoseMovement::PlayTransitionAnimation(%cornerb_stand2crouch_right, "crouch", "stop", 0);
	}

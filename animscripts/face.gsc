...
{
	//Hack around crack jumpers in Sewer
	if (self.voice == "german" && typeString == "giveupposition")
		return;
...
PlayFaceThread(facialanim, soundAlias, importance, notifyString, waitOrNot, timeToWait)
{
	...
	{
		//("Face: Waiting to play sound: ",soundAlias,", anim: ",facialanim,", ", notifyString,(", importance "+importance+", old one "+self.anim_currentDialogImportance));#/
	...
	else if (importance >= self.anim_currentDialogImportance)
	{
		...
		//{	
			//("Face: Interrupted facial sound/animation: ",self.anim_currentDialogSound,", ",self.anim_currentDialogNotifyString, ", ",self.anim_currentDialogImportance);#/
		//}
		if ( isDefined(notifyString) )
		{
...
//			}
	...
	else
	{
		...
		//("Face: Didn't play facial sound: ",soundAlias,", animation: ",facialanim," notify: ",notifyString,", importance ",importance,", old one ",self.anim_currentDialogImportance);#/
	}
}

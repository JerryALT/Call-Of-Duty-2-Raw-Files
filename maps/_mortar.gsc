...
duhoc_style_mortar()
{
	//	script_structs are placed in the level and grouped with script_mortargroup
	...
	groups = [];
	groupNum = [];
	assert(isdefined(level.struct_targetname["mortar"]));
	for (i=0;i<level.struct_targetname["mortar"].size;i++)
	{
		if (!isdefined (level.struct_targetname["mortar"][i].script_mortargroup))
			continue;
		
		index = -1;
		groupNumber = int(level.struct_targetname["mortar"][i].script_mortargroup);
		...
		groups[index][groups[index].size] = level.struct_targetname["mortar"][i];
	}
	
	for (i=0;i<groups.size;i++)
		thread duhoc_style_mortar_think(groups[i]);
}

duhoc_style_mortar_think(mortars, groupNum)
{
	min = 0.5;
	max = 3;
	...
	for (;;)
	{
		...
		level thread duhoc_style_mortar_activate(mortars, groupNum, min, max);
	}
}

duhoc_style_mortar_activate(mortars, groupNum, min, max)
{
	...
	for (;;)
	{
		...
		if ( (isdefined(level.noMortars)) && (level.noMortars == true) )
			return;
		mortars[rand] thread duhoc_style_mortar_explode();
		
		wait (min + randomfloat(max - min));
	}
}

duhoc_style_mortar_checkPlayerFOV()
{
...
duhoc_style_mortar_explode(instant, customExploSound)
{
	if(!isdefined(instant))
		instant = false;
	
	self thread duhoc_style_mortar_cooldown();
	if (!instant)
		self playSoundinSpace (level.scrsound["mortar"]["incomming"]);
	if(isdefined(customExploSound))
		self thread playSoundinSpace (customExploSound);
	else
		self thread playSoundinSpace (level.scrsound["mortar"][self.script_fxid]);
	
	//dont play an effect if it's behind the player - trying to save on particle effects
	radiusDamage ( self.origin, 250, 100, 100);
	if (self duhoc_style_mortar_checkPlayerFOV())
		playfx (level._effect["mortar"][self.script_fxid], self.origin);
	
	if (getcvarint("duhoc_camerashake") > 0)
	{
		//if player is trying to snipe dont do view shake
		if ( (level.player getCurrentWeapon() == "springfield") && (level.player playerADS() > .8) )
			return;
		earthquake(0.25, 2, self.origin, 1250);
	}
}

duhoc_style_mortar_cooldown()
{
	self.cooldown = true;
...
mortar_sound(bIsstruct)
{
	...
	if(!bIsstruct)
		...
		playsoundinspace ("mortar_explosion"+soundnum,self.origin);
}
...
incoming_sound(soundnum , bIsstruct)
{
	...
	{
		if(bIsstruct)
			thread playsoundinspace("mortar_incoming1",self.origin);
		else
	...
	{
		if(bIsstruct)
			thread playsoundinspace("mortar_incoming2",self.origin);
		else
	...
	{
		if(bIsstruct)
			thread playsoundinspace("mortar_incoming3",self.origin);
		else

main()
{
	if (getcvar("debug_vehiclegod") == "")
		setcvar("debug_vehiclegod", "off");
	if (getcvar("debug_vehicleplayerhealth") == "")
		setcvar("debug_vehicleplayerhealth", "off");
	if (getcvar("player_vehicle_dismountable") == "")
		setcvar("player_vehicle_dismountable", "off");
...
binocular_handle()
{
	...
	while ( true )
	{
		msg = level.player maps\_utility::waittill_any( "binocular_enter", "binocular_drop" );
		
		switch ( msg )
		{
		case "binocular_enter":
			thread binocular_enter();
			break;
		case "binocular_drop":
////			self fadeovertime(0);
			self.alpha = 1;
...
binocular_enter()
{
	level.player endon ("binocular_drop");
	wait .3;
	self fadeovertime(0.04);
	self.alpha = 0;
}
...
vehicle_wait(startinvehicle)
{
	...
	{
		if(getcvar("player_vehicle_dismountable") == "off")
		...
		overlay.x = 0;
		overlay.y = 0;
		overlay setshader ("goggles_overlay", 640, 480);
		overlay.alignX = "left";
		overlay.alignY = "top";
		overlay.horzAlign = "fullscreen";
		overlay.vertAlign = "fullscreen";
		overlay.alpha = 1;
//		overlay thread binocular_handle();
	}
	self endon ("death");
	while (self.health > 0)
	{
...
vehicle_enter()
{
	...
	if ( (self.vehicletype == "tiger") || (self.vehicletype == "crusader") || (self.vehicletype == "crusader_player") || (self.vehicletype == "panzer2") || self.vehicletype == "flak88" || self.vehicletype == "flak88_forward")
		self thread vehicle_hud_tank();
...
//	if ( (self.vehicletype == "tiger") || (self.vehicletype == "crusader") || (self.vehicletype == "panzer2") )
//	{
		if(skill == ("easy"))
			self.health  = 3000;
...
vehicle_hud_tank()
{
	...
	{
		self setmodel ("xmodel/vehicle_crusader2_viewmodel");
	}

	regentimer = gettime();
	if (getcvar("debug_vehiclegod") != "off")
	{
		while(1)
		{
	...
	while(1)
	{
		...
		if(self.damaged)
		{
			if(getcvar("debug_vehicleplayerhealth") != "off")

...
main()
{
	...
	maps\_utility::precache ("xmodel/wood_plank2");
	maps\_utility::precache ("xmodel/gib_woodplank");
	...
	if (randomint(100) > 25)
	{
		if ( (isdefined (self.script_noteworthy)) && (self.script_noteworthy == "dark") )
		{
			if (randomint(100) > 50)
				...
				splinter setmodel ("xmodel/gib_woodplank");
		}
		else
		{
			if (randomint(100) > 50)
				splinter setmodel ("xmodel/woodgib_big");
			else
				splinter setmodel ("xmodel/woodgib_medium");
		}
	}
...
small_gibs(org, startorg)
{
	...
	for (i=0;i<randomint(6) + 1;i++)
	{
		...
		if (randomint(100) > 50)
			splinter[i] setmodel ("xmodel/woodgib_small1");
		else
			splinter[i] setmodel ("xmodel/woodgib_small2");

		startorg += (50 - randomint (100),50 - randomint (100),0);
		temp_vec = vectornormalize (org - startorg);
		temp_vec = maps\_utility::vectorScale (temp_vec, 300 + randomint (150));
